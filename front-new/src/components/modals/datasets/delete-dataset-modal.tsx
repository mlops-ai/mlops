...
const DeleteDatasetModal = () => {
    ...
    const isModalOpen = isOpen && type === "deleteDataset";
    ...
    const handleDeleteDataset = async () => {
        ...
        setIsLoading(true);
        await axios
            .delete(`${url}:${port}/datasets/${data.dataset?._id}`)
            .then(() => {
                dataStore.deleteDataset(data.dataset?._id as string);
                onClose();
                createToast({
                    id: "delete-dataset",
                    message: `Dataset ${data.dataset?.dataset_name} deleted successfully!`,
                    type: "success",
                });
            })
            .catch((error: any) => {
    ...
    const onInteractOutside = (e: any) => {
        if (isLoading || toast.isActive("delete-dataset")) {
            e.preventDefault();
        }
    };
            ...
            >
                ...
                <div className="flex flex-col px-4 py-2">
                    <div className="flex items-center">
                        ...
                        <div>
                            Deleting a dataset involves deleting all dataset
                            references to the iterations.{" "}
