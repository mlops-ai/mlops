...
const EditExperimentModal = () => {
    ...
    const isModalOpen = isOpen && type === "editExperiment";
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        let data_to_update =
            data.experiment.name === values.name
                ? { description: values.description }
                : values;

        await axios
            .put(
                `${url}:${port}/projects/${data.project?._id}/experiments/${data.experiment?.id}`,
                data_to_update
            )
            .then((res) => {
    ...
    const onInteractOutside = (e: any) => {
        if (isLoading || toast.isActive("edit-experiment")) {
            e.preventDefault();
        }
    };
