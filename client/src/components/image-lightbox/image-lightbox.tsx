...
import DownloadBold from "../icons/download-bold";
...
export default class Lightbox extends React.Component {
    ...
    navigateImage = (direction, e) => {
        ...
        this.setState({current, x: 0, y: 0, zoom: 1, rotate: 0, loading: false});
                ...
                <div className="lb-header" style={{justifyContent: buttonAlign}}>
                    <Cond condition={showTitle && title}>
                        <div className="lb-title"
                            ...
                            <span title={title}
                                  style={{textAlign: buttonAlign === "flex-start" ? "right" : "left"}}>{title}</span>
                        </div>
                    </Cond>
                    ...
                    <div title="Save Chart" className="flex items-center justify-center p-0 w-[50px] h-[50px] hover:bg-[#777] cursor-pointer"
                         onClick={e => this.saveChart()}>
                        {/* <span className="material-symbols-rounded fw-bold" style={{color: '#fff'}}>
                            download
                        </span> */}
                        <DownloadBold className="font-bold text-white" />
                    </div>
                    <Cond condition={multi}>
                        <div title="Previous" className="lb-button lb-icon-arrow prev lb-hide-mobile"
                             onClick={e => this.navigateImage("prev", e)}></div>
                        <div title="Next" className="lb-button lb-icon-arrow next lb-hide-mobile"
                             onClick={e => this.navigateImage("next", e)}></div>
                    </Cond>
                    <Cond condition={allowZoom}>
                        <div title="Zoom In" className="lb-button lb-icon-zoomin zoomin"
                    ...
                    <Cond condition={allowRotate}>
                        <div title="Rotate left" className="lb-button lb-icon-rotate rotatel"
                             onClick={() => this.applyRotate("acw")}></div>
                        <div title="Rotate right" className="lb-button lb-icon-rotate rotater"
                             onClick={() => this.applyRotate("cw")}></div>
                    </Cond>
                    <div title="Close" className="lb-button lb-icon-close close"
                         style={{order: buttonAlign === "flex-start" ? "-1" : "unset"}}
                         onClick={e => this.exit(e)}></div>
                </div>
                <div
                    ...
                    <div className="mobile-controls lb-show-mobile">
                        {multi ? <div title="Previous" className="lb-button lb-icon-arrow prev"
                                      onClick={e => this.navigateImage("prev", e)}></div> : null}
                        {_reset ? <div title="Reset" className="lb-button lb-icon-reset reload"
                                       onClick={this.reset}></div> : null}
                        {multi ? <div title="Next" className="lb-button lb-icon-arrow next"
                                      onClick={e => this.navigateImage("next", e)}></div> : null}
                    </div>
                </div>
            </div>
        )
